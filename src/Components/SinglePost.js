...
const SinglePost = () => {
  ...
  let URL = `${process.env.REACT_APP_HOST}/posts/${id}`;
  ...
  if (thisPost?.tags && thisPost.tags.length > 1) {
    ...
    post.map((p) => {
      p.tags.map((t) => {
        if (currentPostTags.includes(t) && !relPost.includes(p.title))
          relPost.push(p);
      });
  ...
  if (!thisPost) {
    ...
    return (
      <div className="singlepostwrapper">
        ...
        <Link to="/posts/">
          <button className="backtoposts">back to posts</button>
        </Link>
      </div>
    );
  }
